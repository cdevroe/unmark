...
<div class="main-content">
    ...
    <p><input type="text" size="60" class="input-large" id="emailaddress" name="emailaddress" placeholder="coworker@yourjob.com" /> <?php echo form_submit('btn_invite_member','Invite','class="btn"');?></p>
    ...
    <div class="members">
        ...
        <div class="member">
        <p><?php if ($member['email'] == $this->session->userdata('emailaddress')) { ?><?php echo $member['email']. ' - This is you!'; } else { ?> <a href="<?php echo site_url();?>groups/<?php echo $group['groupuid'];?>/remove/<?php echo $member['id'];?>" class="btn" title="Remove this member"><i class="icon-ban-circle"></i></a> <strong><?php echo $member['email'];?></strong> <small>member since <?php echo date("d F Y",strtotime($member['datejoined']));?></small><?php } ?></p>
        </div>
    </div>
    <?php } ?>
